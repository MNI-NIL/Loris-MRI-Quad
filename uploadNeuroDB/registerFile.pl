...
#@USES       : NeuroDB::DBI and NeuroDB::MRI (and, of course, strict)
#@REQUIRES   : 
#@VERSION    : $Id: registerFile.pl,v 1.3 2007/06/13 18:08:11 sebas Exp $
...
if(defined($source_list)) {
	...
	($center_name, $centerID) = NeuroDB::MRI::getPSC($file->getParameter($lookupCenterNameUsing), \$dbh);
	...
	# get session ID
	my $requiresStaging;
	($sessionID, $requiresStaging) = NeuroDB::MRI::getSessionID($subjectIDsref, $file->getParameter('study_date'), \$dbh, $subjectIDsref->{'subprojectID'}, 1);
	$file->setFileData('SessionID', $sessionID);
...
if($coordinateSpace eq 'native' && !defined($acquisitionProtocol)) {
    ...
    $acquisitionProtocol = &NeuroDB::MRI::identify_scan_db($center_name, $subjectIDsref->{'subprojectID'}, \$file, \$dbh);
}

# save the protocol
if(defined($acquisitionProtocol)) {
    # convert the textual scan_type into the scan_type id
    $acquisitionProtocolID = &NeuroDB::MRI::scan_type_text_to_id($acquisitionProtocol, \$dbh);
